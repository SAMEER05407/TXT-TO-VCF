...
def allow_user(update: Update, context: CallbackContext):
    ...
    if context.args:
        try:
            ...
            update.message.reply_text(f"✅ User {uid} allowed!")
        except:
            update.message.reply_text("❌ Invalid user ID")
    else:
        update.message.reply_text("❌ Usage: /allow <user_id>")
    ...
    if context.args:
        try:
            ...
            if uid in ALLOWED_USERS:
                ...
                save_allowed_users(ALLOWED_USERS)
                update.message.reply_text(f"❌ User {uid} access revoked.")
            else:
...
def main():
    ...
    dp.add_handler(CommandHandler("allow", allow_user))
