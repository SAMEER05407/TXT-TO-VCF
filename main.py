    ...
    if context.args:
        try:
            ...
            if rem_user in ALLOWED_USERS:
                ...
                save_allowed_users(ALLOWED_USERS)
                update.message.reply_text(f"❌ User {rem_user} removed!")
            else:
                update.message.reply_text("User not in list.")
        except:
            update.message.reply_text("❌ Invalid ID")
    else:
...
def process_file(update: Update, context: CallbackContext):
    try:
        ...
        for i in range(0, len(numbers), contacts_per_file):
            ...
            percent = f"{min(100, int((i + len(batch)) / len(numbers) * 100))}%"
        ...
        msg.edit_text(f"✅ Done! Total {len(numbers)} contacts.")
